    ...
    return (
      <div className="detail-container">
        <div className="detail-error">
          <h2>Film / Série introuvable</h2>
          <Link to="/movies" className="back-home-btn">Retour aux films</Link>
        </div>
      </div>
    );
  }

  const characterCount = movie.characters?.length || 0;
  const attributeCount = movie.attributes?.length || 0;
  const previewCharacters = movie.characters?.slice(0, 8) || [];

  return (
    <div
      className="detail-container"
      style={{ backgroundImage: `url(${API_URL}/api/images/movies/${id}/${movie.background})` }}
    >
      <div className="detail-overlay">
        {/* Navigation */}
        <div className="detail-nav">
          <Link to="/movies" className="back-home-btn">← Films & Séries</Link>
        </div>

        {/* Header */}
        <header className="detail-header">
          <div className="detail-icon">
            <img src={`${API_URL}/api/images/movies/${id}/${movie.icon}`} alt={movie.name} />
          </div>
          <div className="detail-title-section">
            ...
            <div className="detail-stats">
              ...
              <div className="stat-badge">
                <span className="stat-number">{movie.maxAttempts || 26}</span>
        ...
        <section className="detail-section">
          ...
          <div className="game-modes">
            <Link to={`/movie/${id}/play`} className="mode-card solo">
            ...
            <Link
              to={`/movie/${id}/play?mode=duo`}
              className={`mode-card duo ${!user && !authLoading ? 'requires-login' : ''}`}
            >
        ...
        {characterCount > 0 && (
          <section className="detail-section">
            <div className="section-header">
              ...
              <Link to={`/movie/${id}/characters`} className="see-all-btn">
                Voir tous →
              </Link>
            </div>
            <div className="characters-preview">
              {previewCharacters.map((char) => (
                <div key={char.id} className="character-preview-card">
                  <img
                    src={`${API_URL}/api/images/movies/${id}/characters/${char.image}`}
                    alt={char.name}
